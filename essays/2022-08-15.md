...
Now that both [HOPE](https://xiv.hope.net/) and [DEF CON](https://defcon.org/html/defcon-30/dc-30-index.html) are behind me, I want to summarize some of my initial thoughts. This is not a "what did I learn about hacking" post, but rather musings about *attending* these events.
...
After much consideration, I've come to believe that much of this "advice" is ill-conceived, and shows either the advice-giver's poor risk modeling abilities or inflated sense of their own importance. [Rob Graham's advice](https://blog.erratasec.com/2019/08/securing-devices-for-defcon.html) is much better: Encrypt your stuff and be careful about Wi-Fi and Bluetooth, and you'll almost certainly be fine.
...
- I *don't* take my regular computer with me. I travel with a recent [iPad Pro](https://www.apple.com/ipad-pro/) and [iPhone](https://www.apple.com/iphone/), and also bring along a [Raspberry Pi](https://www.raspberrypi.com/) (currently a 4B running [Kali Linux](https://www.kali.org/)) in case I need tools I can't run on the iPad (this is actually less common than you'd think).
...
- I have [Find My](https://www.apple.com/icloud/find-my/) active on all of my devices so if the device is stolen I can lock/wipe it remotely.
- Finally, I use [Proton VPN](https://protonvpn.com/) on all my devices. I do this not for "privacy" reasons, but because using a VPN is a way to ensure that any applications that are *not* using encrypted connections or are vulnerable to downgrade attacks have an extra layer of protection. That Proton VPN also provides some on-the-wire malware blocking is an added bonus.

Now, hacker conferences *do* generally have a more hostile radio frequency environment, so I do take extra care here.

- I turn off Bluetooth and leave it off attending the conference. Since I only travel with wired headphones, this isn't a big hardship for me. The only time I turn on Bluetooth is if I want to edit any photography (I have an [Apple Pencil](https://www.apple.com/apple-pencil/) for my iPad), but I only do this when away from the conference area.
- I remove all Wi-Fi networks from my devices (which means deleting them from iCloud Keychain). For DEF CON, I configure [the secure conference Wi-Fi](https://wifireg.defcon.org/) before leaving. That said, I keep Wi-Fi turned off on my devices as well unless I absolutely need it.
...
I use different usernames/emails for almost every service. Both [Proton Mail](https://proton.me/support/catch-all) and [Google Workspace](https://support.google.com/a/answer/2685650) can be configured with catch-all email addresses. If you don't want to pay money, then sign up for Gmail and liberally use [the `+` functionality](https://support.google.com/a/users/answer/9308648).

Use unique, random passwords for as many services as you can. Use a password manager ([KeePassXC](https://keepassxc.org) and compatible applications are good choices) to make this easy to manage. Don't store these credentials in your browser or system keychain.

Set up multi-factor authentication on *every* account that supports it. Whenever possible, use a hardware key (like a [YubiKey](https://www.yubico.com/products/yubikey-5-overview/)). As a bonus, you can use hardware security keys to further lock down access to your password manager: Do this using either [built-in support](https://keepassxc.org/docs/KeePassXC_UserGuide.html#_database_settings), or by storing a long random string in one of your security key "slots" and then using this *in addition to* a string that you've memorized to access your password vault (full-featured YubiKeys - not the lower-end "security keys" Yubico sells - support both options).

## Packing

I'm a light packer, partly because I don't like checking bags, and partly because I enjoy the challenge. This year I managed to fit all of my gear into a single [Peak Design 20 L Everyday Backpack](https://www.peakdesign.com/products/everyday-backpack) (with the help of two small [packing cubes](https://www.peakdesign.com/products/packing-cube/)). However, this is all a tight fit - there's next to no room for conference swag on the return trip, and the laptop compartments are compressed more than I'm comfortable with. Moving forward, I intend to switch to a Peak Design 30 L Everyday Backpack, as I think that the extra 10 L should be more than enough for me.
...
One thing that's enables me to pack light is that I've moved away from wearing jeans. Instead, I've switched to [Bluffworks' Ascender 5-Pocket Pants](https://shop.bluffworks.com/products/ascender-5-pocket-pants-regular-fit-asphalt-black), which are tough, light, wrinkle-resistant, *very* compressible, and have the bonus of being much harder to pick pocket than most other mens' pants (though again, a determined thief can still defeat them). I pair these with Icebreaker [underwear](https://www.icebreaker.com/en-us/mens-underwear) and [socks](https://www.icebreaker.com/en-us/mens-socks), which are also exceptionally light and durable. Bluffworks also makes [travel blazers](https://shop.bluffworks.com/products/gramercy-blazer-classic-fit-blue-hour) and [other outfits for less casual situations](https://shop.bluffworks.com/pages/performance-suits); these are also good for traveling light, but by necessity take up a bit more space. There's a trade-off between looking nice and packing light that is just difficult to navigate.
...
I don't like wearing masks, but if I'm going to wear one I'm going to wear an *effective* one. Since HOPE and DEF CON were both masked events, I brought along tight-fitting [N95 masks](https://bnx.com/products/n95-mask-black-made-in-usa-bifold-h95b/) for use while attending the conference.
