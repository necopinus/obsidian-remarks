...
aliases:
  ...
  - September 27th, 2022
  - Sunday September 27th 2022
---
# Trying (and failing) to deploy a smart contract using an iPad Pro

My personal project this weekend was to deploy a smart contract to the Ethereum blockchain. The motivation here was to be able to mint an NFT (really, several NFTs) for use as an avatar on [Farcaster](https://www.farcaster.xyz/), an alternative to Twitter that [I've been experimenting with](https://fcast.me/necopinus) for a few weeks now. Using an NFT as your avatar on Farcaster adds a "[purple checkmark](https://farcasterxyz.notion.site/How-to-get-a-purple-checkmark-fb66f0cb0f5f4f24b699b8f288a2f14a)" to your account, a spoof on Twitter's infamous [blue checkmark](https://help.twitter.com/managing-your-account/about-twitter-verified-accounts).

(For those who are interested, [Twitter provides the ability to set an NFT as your avatar](https://support.opensea.io/hc/en-us/articles/4415562648851-How-do-I-set-my-NFT-as-my-Twitter-profile-picture-), though you need to subscribe to "Twitter Blue" and be using iOS in order to access this feature.)

The central problem here is that if I use an NFT as my avatar, then I need to pay for another NFT (either to buy one or mint one myself) whenever I decide I want to change my avatar. Also, my current (and favorite) avatar is taken from a [John Schoenherr](https://dangerousminds.net/comments/the_artist_who_visited_dune) painting of one of the great sandworms of Arrakis - I'm not sure I want to "mint" that in any way that could be interpreted as me claiming "ownership" of it. It's Schoenherr's painting after all!

Finally, all of the services I've seen that allow you to mint your own NFT store the associated JSON metadata and image file using a decentralized solution like [IPFS](https://ipfs.tech/) or [Arweave](https://www.arweave.org/). This is actually a good thing, because it gives "collectable" NFTs a degree of permanence, but in my case I actually *want* to be able to update the associated metadata and image arbitrarily. Fortunately, I control my own domain and [EIP-721](https://eips.ethereum.org/EIPS/eip-721) (which defines the NFT standard) doesn't forbid using "non-permenant" URIs, so all this means is that I need to deploy my own smart contract to create my NFT avatar(s), rather than using a pre-packaged solution.
...
While working on the iPad, I used Safari with the [Light Wallet](https://wallet.light.so) extension for interacting with the Ethereum blockchain.

As fate would have it, I started using a standard location for my avatar (https://necopinus.xyz/​avatar/​avatar.webp) when I set up my ENS domains. For some reason the correct URL isn't showing up on [the ENS console](https://app.ens.domains/name/necopinus.xyz/details), even though it appears to be set when I view it in the controlling [Safe](https://app.safe.global/) app. I'm not sure why this is, but the important thing is that I already have a standard location for my avatar.
...
Once this was done, I decided to more-or-less follow the "[How to Develop an NFT Smart Contract (ERC721) with Alchemy](https://docs.alchemy.com/docs/how-to-develop-an-nft-smart-contract-erc721-with-alchemy)" walk-through. I'm going to skip a lot of the details here, so if you're trying to do this yourself you should also read through that document.

I also tested deploying the smart contract and minting my NFTs on the Goerli TestNet before committing any money to deploy on the Ethereum MainNet. To do this, you'll need to get some GoerliETH; the easiest way to do this is to create an account on [Alchemy](https://www.alchemy.com) and then visit [the Goerli "faucet"](https://goerlifaucet.com/), which will transfer 0.2 GoerliETH to your wallet (more than enough for what follows.

Next, go to the [Open Zeppelin Contracts Wizard](https://docs.openzeppelin.com/contracts/4.x/wizard) to pre-generate smart contract code. I'm going to select an ERC721 contract and select the "Mintable, Autoincrement IDs", "Enumerable", and "URI Storage" options + uncreative contract and token names. This produces the following code.
...
Eventually I installed [Brave](https://brave.com/) on my MacBook for the sole purpose of being able to deploy the smart contract code. [Brave's built-in wallet](https://brave.com/wallet/) works perfectly with Remix.
...
The minted NFTs took a looooong time to show up in [MetaMask](https://metamask.io), but they did show up in [OpenSea](https://opensea.io) pretty much immediately. You can use the OpenSea interface to transfer the minted NFTs to other accounts.
