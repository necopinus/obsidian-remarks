...
As usual, we'll start out with an [Nmap](https://nmap.org) scan:
...
The next few flags are about finding a particular vulnerability for Icecast on [https://cvedetails.com](https://cvedetails.com). There's actually a couple of different vulnerabilities that match the flag descriptions, but after a little trial-and-error we can find the [right one](https://www.cvedetails.com/cve/CVE-2004-1561/).

There's not too much to the corresponding [Metasploit](https://www.metasploit.com/) module - set RHOSTS and LHOST, run, get shell.
...
We're going to loot LSASS now using [Mimikatz](https://github.com/gentilkiwi/mimikatz).
...
Hashes dumped using hashdump can be cracked offline using [Hydra](https://github.com/vanhauser-thc/thc-hydra) or [John the Ripper](https://www.openwall.com/john/). It can be helpful to know a little about [Kerberos](https://en.wikipedia.org/wiki/Kerberos_%28protocol%29) tickets at this point
