...
> [!note]
> ["Retro" on TryHackMe](https://tryhackme.com/room/retro)

## Initial reconnaissance

This CTF is an earlier version of [[2021-12-08|Blaster]] without the step-by-step nature of that room. Not 100% sure what the differences are.

The target IP is 10.10.20.185.

As with Blaster, we'll start out by running an [Nmap](https://nmap.org/) scan:
...
As with Blaster, we just have the default IIS welcome page. So let's follow up with [gobuster](https://github.com/Oj/gobuster) since we know from the flag list that there's a hidden directory.
...
All of the posts are by "Wade". I hadn't noticed it the first time, but the "[Ready Player One](https://en.wikipedia.org/wiki/Ready_Player_One)" post is actually suggestive that either Wade's username or password is "Parzival".
