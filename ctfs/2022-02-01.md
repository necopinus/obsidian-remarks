...
> [!note]
> ["Inclusion" on TryHackMe](https://tryhackme.com/room/inclusion)

This room is intended to focus on local file inclusion attacks, so my guess is that hitting the machine with [Nmap](https://nmap.org/) and [gobuster](https://github.com/Oj/gobuster) is probably unnecessary. But, I'm going to do it anyway, just in case.
...
While it's probably *pro forma*, let's run our usual [Nmap](https://nmap.org/) scan just in case:
...
We'll also hit 10.10.0.57 with [gobuster](https://github.com/Oj/gobuster):
