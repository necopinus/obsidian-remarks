...
> [!note]
> ["Attacktive Directory" on TryHackMe](https://tryhackme.com/room/attacktivedirectory)

## Welcome to Attacktive Directory

As usual, we start off with an [Nmap](https://nmap.org/) scan. Our target IP address is 10.10.177.198.
...
We'll use [enum4linux](https://github.com/cddmp/enum4linux-ng) to gather some additional information from the Samba shares (139/445) on the target.
...
The AD domain itself is using the standard (invalid) .local TLD, but that's not actually something that [enum4linux](https://github.com/cddmp/enum4linux-ng) tells us - instead, we can see this in the earlier output from [Nmap](https://nmap.org/).

## Enumerating users via Kerberos

We'll now try to enumerate users using [Kerbrute](https://github.com/ropnop/kerbrute) and [the provided user list](https://github.com/Sq00ky/attacktive-directory-tools/blob/master/userlist.txt).
...
We're going to try [AS-REP Roasting](https://blog.certcube.com/as-rep-roasting-attack/) using [Impacket](https://github.com/fortra/impacket)'s GetNPUsers.py script. We first create an `attacktive-directory.targets` file containing the following:
...
GetNPUsers.py reports that backup isn't vulnerable to AS-REP roasting, but svc-admin *is* vulnerable and we get back a valid [Hashcat](https://hashcat.net/hashcat/) password hash.

A quick lookup in the [Hashcat example hashes wiki page](https://hashcat.net/wiki/doku.php?id=example_hashes) shows that this is a "[Kerberos](https://en.wikipedia.org/wiki/Kerberos_%28protocol%29) 5, etype 23, AS-REP" hash ([Hashcat](https://hashcat.net/hashcat/) mode 18200). We'll crack this using [the provided password list](https://github.com/Sq00ky/attacktive-directory-tools/blob/master/passwordlist.txt).
...
The CTF now reveals that the backup account has all AD changes synced to it, including NT hashes, and that we can use [Impacket](https://github.com/fortra/impacket)'s secretsdump.py file to obtain these from NTDS.DIT.
...
We'll use the [Evil-WinRM](https://github.com/hackplayers/evil-winrm) tool to retrieve the final flag by [passing the hash](https://en.wikipedia.org/wiki/Pass_the_hash).
