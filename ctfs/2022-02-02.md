...
> [!note]
> ["Jurassic Park" on TryHackMe](https://tryhackme.com/room/jurassicpark)
...
- Trying to include /etc/os-release via item.php causes the site to display a Dennis Nedry page that taunts you to use [SQLMap](https://sqlmap.org/). Trying a hand-crafted test SQL injection also displays this page.
...
Let's see if our usual [Nmap](https://nmap.org/) scan turns up anything interesting:
...
Let's also hit the target with [gobuster](https://github.com/Oj/gobuster):
...
What the hell, let's throw [SQLMap](https://sqlmap.org/) at it anyway!
